...
RESULT_JSON = Path("live_stop_tightening_probe.json")
RESULT_MD = Path("LIVE_STOP_TIGHTENING_PROBE_REPORT.md")
...
def main():
    ...
    try:
        ...
        entry = runtime.order_executor.submit_order(
            ...
            quantity=quantity,
            reduce_only=False,
            metadata={"live_stop_tightening_probe": True},
        )
        if not entry:
        ...
        # Choose a tighter stop that stays below the actual market price to avoid immediate trigger.
        target_stop_price = max(
            initial_managed_stop,
            actual_market_price * 0.995,
        )
        if target_stop_price >= actual_market_price:
            target_stop_price = actual_market_price * 0.99
        ...
        close_result = runtime.order_executor.submit_order(
            ...
            reduce_only=True,
            metadata={"entry_price": entry_price, "live_stop_tightening_probe": True},
        )
        if close_result:
