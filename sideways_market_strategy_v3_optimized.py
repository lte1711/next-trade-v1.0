#!/usr/bin/env python3
"""
횡보장 전략 v3 백테스트 실행기 - 최적화 파라미터 버전
실제 데이터로 백테스트 실행 및 파라미터 튜닝
"""
...
@dataclass
class RiskConfig:
    risk_per_trade: float = 0.0008     # 0.08% of equity (최적화)
    atr_stop_mult: float = 3.0        # 3.0x ATR (최적화)
    atr_take_mult: float = 2.5        # 2.5x ATR (최적화)
    fee_rate: float = 0.0004          # 0.04% per side
    slippage_rate: float = 0.0002     # 0.02% per side
    max_holding_bars: int = 6         # 6 bars (30분, 최적화)
...
class StrategyConfig:
    ...
    rsi_period: int = 14
    bb_period: int = 20
    bb_std: float = 1.3              # 1.3 (최적화)
    z_period: int = 20
    ranging_adx_threshold: float = 35.0  # 35.0 (최적화)
    trending_adx_threshold: float = 40.0  # 40.0 (최적화)
    bandwidth_quantile_window: int = 100
    bandwidth_ranging_quantile: float = 0.5  # 0.5 (최적화)
    rsi_overbought: float = 55.0     # 55.0 (최적화)
    rsi_oversold: float = 45.0        # 45.0 (최적화)
    z_entry_threshold: float = 1.2      # 1.2 (최적화)
    volume_lookback: int = 20
    low_volume_factor: float = 0.5     # 0.5 (최적화)
    tr_spike_atr_mult: float = 3.0     # 3.0 (최적화)
    realized_vol_window: int = 20
    realized_vol_spike_mult: float = 1.5
    ...
    def _signal_row(self, row: pd.Series) -> pd.Series:
        ...
        if pd.notna(close) and pd.notna(lower) and pd.notna(rsi):
            if close <= lower and rsi <= self.cfg.rsi_oversold:
                ...
                rsi_score = min(1.0, max(0.0, (self.cfg.rsi_oversold - rsi) / 50.0))
                adx_score = 0.0 if pd.isna(adx) else min(1.0, max(0.0, (self.cfg.ranging_adx_threshold - adx) / 35.0))
                conf = self._confidence_clip(0.50 + 0.35 * rsi_score + 0.10 * distance_score + 0.05 * adx_score)
                return pd.Series(
                    {
                        "signal": "BUY",
                        "signal_source": "RANGE",
        ...
        if pd.notna(close) and pd.notna(upper) and pd.notna(rsi):
            if close >= upper and rsi >= self.cfg.rsi_overbought:
                ...
                rsi_score = min(1.0, max(0.0, (rsi - self.cfg.rsi_overbought) / 50.0))
                adx_score = 0.0 if pd.isna(adx) else min(1.0, max(0.0, (self.cfg.ranging_adx_threshold - adx) / 35.0))
                conf = self._confidence_clip(0.50 + 0.35 * rsi_score + 0.10 * distance_score + 0.05 * adx_score)
                return pd.Series(
                    {
        ...
        # Mean reversion fallback - 최적화
        if pd.notna(z):
            if z <= -self.cfg.z_entry_threshold:
                z_score = min(1.0, abs(z) / (self.cfg.z_entry_threshold + 0.8))
                pb_score = 0.0 if pd.isna(pb) else min(1.0, max(0.0, (0.05 - pb) / 0.05))
                conf = self._confidence_clip(0.45 + 0.50 * z_score + 0.05 * pb_score)
                return pd.Series(
                    {
                        "signal": "BUY",
            ...
            if z >= self.cfg.z_entry_threshold:
                z_score = min(1.0, abs(z) / (self.cfg.z_entry_threshold + 0.8))
                pb_score = 0.0 if pd.isna(pb) else min(1.0, max(0.0, (pb - 0.95) / 0.05))
                conf = self._confidence_clip(0.45 + 0.50 * z_score + 0.05 * pb_score)
                return pd.Series(
                    {
                        "signal": "SELL",
...
# 메인 실행 함수
def main():
    """메인 실행 함수 - 최적화 파라미터로 백테스트"""
    print("🎯 횡보장 전략 v3 백테스트 시작 (최적화 파라미터)")
    print("=" * 60)
    ...
    # 데이터 일부만 사용 (최근 3개월)
    if len(df) > 25000:  # 약 3개월 분량 (5분데이터)
        df = df.tail(25000)
        print(f"📊 최근 3개월 데이터만 사용: {len(df)} 행")
    ...
    output_dir = f"backtest_results_optimized_{timestamp}"
    ...
    print(f"리스크 비율: {config['risk_config']['risk_per_trade']*100:.1f}%")
    print(f"손절 배수: {config['risk_config']['atr_stop_mult']}")
    print(f"익절 배수: {config['risk_config']['atr_take_mult']}")
    print(f"최대 보유: {config['risk_config']['max_holding_bars']} 봉")
    print(f"ADX 횡보장 기준: {config['strategy_config']['ranging_adx_threshold']}")
    print(f"RSI 과매수: {config['strategy_config']['rsi_overbought']}")
    print(f"RSI 과매도: {config['strategy_config']['rsi_oversold']}")
    print(f"Z-Score 임계값: {config['strategy_config']['z_entry_threshold']}")
    ...
    print("🔄 전체 성과 비교")
    print("-" * 50)
    print("초기 결과: -83.06% 수익률, 26.55% 승률")
    print("개선 결과: -88.21% 수익률, 29.01% 승률")
    print("보수적 결과: -36.58% 수익률, 32.89% 승률")
    print("최적화 결과: {:.2f}% 수익률, {:.2f}% 승률".format(
        summary['total_return_pct'], summary['win_rate']
    ))
    
    improvement_pct = summary['total_return_pct'] - (-83.06)
    win_rate_improvement = summary['win_rate'] - 26.55
    
    print(f"수익률 개선: {improvement_pct:.2f}%")
    print(f"승률 개선: {win_rate_improvement:.2f}%")
    print()
