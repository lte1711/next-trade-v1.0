...
class OrderExecutor:
    ...
    def __init__(self, api_key, api_secret, base_url, trading_results, symbol_info_getter, 
        ...
        self._last_exchange_realized_sync_ms = 0
    ...
    def submit_order(self, strategy_name, symbol, side, quantity, reduce_only=False, metadata=None):
        ...
        try:
            ...
            if reduce_only:
                live_position_amount = self._get_live_position_amount(symbol)
                if live_position_amount is None:
                    self.trading_results.setdefault("operational_events", []).append({
                        "timestamp": datetime.now().isoformat(),
                        "type": "reduce_only_precheck_unavailable",
                        "symbol": symbol,
                        "side": side,
                        "requested_quantity": quantity,
                    })
                elif live_position_amount == 0:
                    self.trading_results.setdefault("operational_events", []).append({
                        "timestamp": datetime.now().isoformat(),
                        "type": "reduce_only_blocked_no_position",
                        "symbol": symbol,
                        "side": side,
                        "requested_quantity": quantity,
                    })
                    return {
                        "status": "BLOCKED",
                        "reason": "reduce_only_no_matching_position",
                    }
                else:
                    closes_long = side == "SELL" and live_position_amount > 0
                    closes_short = side == "BUY" and live_position_amount < 0
                    if not (closes_long or closes_short):
                        self.trading_results.setdefault("operational_events", []).append({
                            "timestamp": datetime.now().isoformat(),
                            "type": "reduce_only_blocked_side_mismatch",
                            "symbol": symbol,
                            "side": side,
                            "live_position_amount": live_position_amount,
                            "requested_quantity": quantity,
                        })
                        return {
                            "status": "BLOCKED",
                            "reason": "reduce_only_side_mismatch",
                        }

                    quantity = min(quantity, abs(live_position_amount))
    ...
    def sync_exchange_realized_pnl(self, lookback_hours=4, min_interval_seconds=240, max_symbols=24):
        """Backfill realized PnL from exchange close orders missed by local order tracking."""
        summary = {
            "checked_symbols": 0,
            "orders_seen": 0,
            "trade_rows_seen": 0,
        ...
        try:
            ...
            if (
                self._last_exchange_realized_sync_ms
                and server_time - self._last_exchange_realized_sync_ms < min_interval_seconds * 1000
            ):
                summary["skipped"] = "throttled"
                return summary
            self._last_exchange_realized_sync_ms = server_time

            start_time = server_time - int(max(1, lookback_hours) * 60 * 60 * 1000)
            known_order_ids = self._known_realized_order_ids()
            symbols = self._symbols_for_exchange_pnl_sync()
            if max_symbols and len(symbols) > max_symbols:
                summary["symbols_total"] = len(symbols)
                summary["symbols_limited_to"] = max_symbols
                symbols = symbols[:max_symbols]
            for symbol in symbols:
                exchange_orders = self._signed_request(
                    "GET",
                    "/fapi/v1/allOrders",
                    {"symbol": symbol, "startTime": start_time, "limit": 1000},
                )
                user_trades = self._signed_request(
                    "GET",
                    "/fapi/v1/userTrades",
                    {"symbol": symbol, "startTime": start_time, "limit": 1000},
                )
                summary["checked_symbols"] += 1

                if not isinstance(exchange_orders, list):
                    summary["errors"].append({"symbol": symbol, "source": "allOrders", "response": exchange_orders})
                    continue
                if not isinstance(user_trades, list):
                    summary["errors"].append({"symbol": symbol, "source": "userTrades", "response": user_trades})
                    continue

                summary["orders_seen"] += len(exchange_orders)
                summary["trade_rows_seen"] += len(user_trades)
                realized_by_order = self._aggregate_realized_pnl_by_order(user_trades)
                orders_by_id = {
                    self._normalize_order_id(order.get("orderId")): order
                    for order in exchange_orders
                    if self._normalize_order_id(order.get("orderId")) is not None
                }

                for order_id, realized_pnl in sorted(realized_by_order.items()):
                    if order_id in known_order_ids:
                        continue
                    if abs(realized_pnl) <= 1e-12:
                        continue

                    exchange_order = orders_by_id.get(order_id)
                    if not exchange_order:
                        continue
                    if not (
                        self._exchange_bool(exchange_order.get("reduceOnly"))
                        or self._exchange_bool(exchange_order.get("closePosition"))
                    ):
                        continue

                    trade_record = self._create_exchange_realized_record(
                        symbol=symbol,
                        order_id=order_id,
                        exchange_order=exchange_order,
                        realized_pnl=realized_pnl,
                    )
                    self.trading_results.setdefault("real_orders", []).append(trade_record)
                    self._record_realized_pnl_event(trade_record)
                    known_order_ids.add(order_id)
                    summary["recorded"] += 1
                    summary["realized_pnl"] = round(summary["realized_pnl"] + realized_pnl, 8)
                    summary["records"].append({
                        "symbol": symbol,
                        "order_id": order_id,
                        "side": trade_record.get("side"),
                        "exit_price": trade_record.get("price"),
                        "executed_qty": trade_record.get("executed_qty"),
                        "realized_pnl": realized_pnl,
                        "exit_reason": trade_record.get("exit_reason"),
                    })

            if summary["recorded"] or summary["errors"]:
                self.trading_results.setdefault("operational_events", []).append({
                    "timestamp": datetime.now().isoformat(),
                    "type": "exchange_realized_pnl_sync",
                    "summary": summary,
                })
            return summary
        except Exception as e:
            self.log_error("exchange_realized_pnl_sync", str(e))
            summary["errors"].append(str(e))
            return summary
    ...
    def _known_realized_order_ids(self):
        """Return order IDs already represented in local realized PnL state."""
        known_order_ids = set()
        for entry in self.trading_results.get("realized_pnl_journal", []):
            order_id = self._normalize_order_id(entry.get("order_id"))
            if order_id is not None:
                known_order_ids.add(order_id)
        for order in self.trading_results.get("real_orders", []):
    ...
    def _symbols_for_exchange_pnl_sync(self):
        """Build a bounded symbol set likely to contain recently closed protective orders."""
        symbols = []
        seen = set()
        def add_symbol(symbol):
            normalized = str(symbol).upper() if symbol else ""
            if normalized and normalized not in seen:
                symbols.append(normalized)
                seen.add(normalized)

        for container_name in ("active_positions", "managed_stop_prices", "position_entry_times", "recently_closed_symbols"):
            container = self.trading_results.get(container_name, {})
            if isinstance(container, dict):
                for symbol in container.keys():
                    add_symbol(symbol)

        for collection_name in ("real_orders", "closed_trades", "realized_pnl_journal"):
            collection = self.trading_results.get(collection_name, [])
            if not isinstance(collection, list):
                continue
            for record in reversed(collection[-250:]):
                symbol = record.get("symbol") if isinstance(record, dict) else None
                add_symbol(symbol)

        return symbols

    def _aggregate_realized_pnl_by_order(self, user_trades):
        """Aggregate exchange user trade realized PnL by order ID."""
        realized_by_order = {}
        for trade in user_trades:
            order_id = self._normalize_order_id(trade.get("orderId"))
            if order_id is None:
                continue
            realized_by_order[order_id] = realized_by_order.get(order_id, 0.0) + self.safe_float_conversion(
                trade.get("realizedPnl"),
                0.0,
            )
        return realized_by_order

    def _create_exchange_realized_record(self, symbol, order_id, exchange_order, realized_pnl):
        """Create an auditable local record for exchange-triggered close orders."""
        update_time = self._normalize_order_id(exchange_order.get("updateTime")) or self._normalize_order_id(exchange_order.get("time"))
        timestamp = datetime.now().isoformat()
        if update_time:
            timestamp = datetime.fromtimestamp(update_time / 1000).isoformat()

        avg_price = self.safe_float_conversion(exchange_order.get("avgPrice"), 0.0)
        executed_qty = self.safe_float_conversion(exchange_order.get("executedQty"), 0.0)
        close_position = self._exchange_bool(exchange_order.get("closePosition"))
        reduce_only = self._exchange_bool(exchange_order.get("reduceOnly"))
        order_type = (
            exchange_order.get("origType")
            or exchange_order.get("type")
            or exchange_order.get("orderType")
            or ""
        )
        order_type = str(order_type).upper()
        exit_reason = "exchange_protective_close"
        if order_type == "STOP_MARKET":
            exit_reason = "exchange_stop_loss_close"
        elif order_type == "TAKE_PROFIT_MARKET":
            exit_reason = "exchange_take_profit_close"
        elif close_position:
            exit_reason = "exchange_close_position"

        return {
            "strategy": self._infer_strategy_for_symbol(symbol),
            "symbol": symbol,
            ...
            "timestamp": timestamp,
            "type": "ACTUAL_TRADE",
            "market_regime": self.trading_results.get("market_regime", {}),
            "strategy_signal": exchange_order.get("side"),
            "position_type": "LONG" if exchange_order.get("side") == "BUY" else "SHORT",
            "reduce_only": reduce_only,
            "close_position": close_position,
            "exchange_order_type": order_type,
            "exchange_update_time": exchange_order.get("updateTime"),
            "realized_pnl": realized_pnl,
            "entry_price": 0.0,
            "exit_reason": exit_reason,
            "source": "exchange_realized_pnl_sync",
        }

    def _infer_strategy_for_symbol(self, symbol):
        """Best-effort strategy lookup for exchange-only close records."""
        active_position = self.trading_results.get("active_positions", {}).get(symbol, {})
        if active_position.get("strategy"):
            return active_position.get("strategy")
        for order in reversed(self.trading_results.get("real_orders", [])):
            if order.get("symbol") == symbol and not order.get("reduce_only") and order.get("strategy"):
                return order.get("strategy")
        return "unknown"

    def _normalize_order_id(self, order_id):
        """Normalize Binance order IDs for de-duplication."""
        try:
            if order_id is None or order_id == "UNKNOWN":
                return None
            return int(order_id)
        except Exception:
            return None

    def _exchange_bool(self, value):
        """Normalize Binance boolean fields that may arrive as bools or strings."""
        if isinstance(value, bool):
            return value
        return str(value).lower() == "true"

    def _signed_request(self, method, path, params=None):
        """Run a signed Binance Futures request."""
        server_time = self.get_server_time()
        if not server_time:
            return {"error": "server_time_unavailable"}

        request_params = dict(params or {})
        request_params["timestamp"] = int(server_time) - 100
        request_params["recvWindow"] = getattr(self, "recv_window", 5000)
    ...
    def _get_live_position_amount(self, symbol):
        """Fetch the latest exchange position amount for reduce-only validation."""
        try:
            server_time = self.get_server_time()
            if not server_time:
                return None
            ...
            if isinstance(payload, list) and payload:
                return self.safe_float_conversion(payload[0].get("positionAmt"), 0.0)
            if isinstance(payload, dict):
                return self.safe_float_conversion(payload.get("positionAmt"), 0.0)
        except Exception:
            pass
        return None
