...
class PositionManager:
    ...
    def manage_profit_targets(self, symbol: str, strategy_config: Dict[str, Any]):
        ...
        try:
            ...
            # Capture profit earlier because recent realized losses are larger
            # than wins; defaults can still be overridden by strategy config.
            partial_levels = risk_config.get('partial_tp_levels', [0.25, 0.5, 0.8])
            close_fractions = risk_config.get('partial_tp_close_fractions', [0.40, 0.35, 1.0])
            
            for i, level in enumerate(partial_levels):
                target_pct = take_profit_pct * level
                close_fraction = close_fractions[i] if i < len(close_fractions) else 1.0
    ...
    def evaluate_fast_reversal(self, symbol: str, indicators: Dict[str, Any], current_price: float) -> Dict[str, Any]:
        """Detect short-timeframe reversals before the 1h trend fully turns."""
        try:
            position = self.trading_results.get("active_positions", {}).get(symbol, {})
            ...
            state = self.get_position_management_state(symbol)
            pnl_pct = self.safe_float_conversion(state.get("pnl_pct"), 0.0)
            mtf = indicators.get("multi_timeframe_market", {}) or {}
            frames = mtf.get("frames", {}) or {}
            ema21 = (indicators.get("ema_data", {}) or {}).get("ema21", []) or []
            current_ema21 = self.safe_float_conversion(ema21[-1] if ema21 else 0.0, 0.0)

            f5 = frames.get("5m", {}) or {}
            f15 = frames.get("15m", {}) or {}
            f30 = frames.get("30m", {}) or {}

            side = "LONG" if amount > 0 else "SHORT"
            confirmed_reversal = False
            if side == "LONG":
                f15_score = self.safe_float_conversion(f15.get("score"), 0.0)
                mtf_score = self.safe_float_conversion(mtf.get("score"), 0.0)
                fast_reversal = (
                    f5.get("direction") == "BEARISH"
                    and f15.get("direction") == "BEARISH"
                    and (current_ema21 <= 0 or current_price < current_ema21 or f15_score <= -0.35)
                )
                confirmed_reversal = fast_reversal and (
                    pnl_pct <= -0.009
                    or (
                        f30.get("direction") == "BEARISH"
                        and (mtf_score <= -0.18 or f15_score <= -0.45)
                    )
                    or (
                        f15_score <= -0.55
                        and mtf_score <= -0.22
                ...
                mtf_score = self.safe_float_conversion(mtf.get("score"), 0.0)
                fast_reversal = (
                    f5.get("direction") == "BULLISH"
                    and f15.get("direction") == "BULLISH"
                    and (current_ema21 <= 0 or current_price > current_ema21 or f15_score >= 0.35)
                )
                confirmed_reversal = fast_reversal and (
                    pnl_pct <= -0.009
                    or (
                        f30.get("direction") == "BULLISH"
                        and (mtf_score >= 0.18 or f15_score >= 0.45)
                    )
                    or (
                        f15_score >= 0.55
            ...
            if not fast_reversal:
                return {
                    "action": "hold",
                    "reason": "fast_reversal_not_confirmed",
                    "side": side,
                    "pnl_pct": round(pnl_pct, 6),
                }

            action = "close" if confirmed_reversal else "partial"
            return {
                "action": action,
                "reason": "confirmed_fast_reversal" if confirmed_reversal else "early_fast_reversal",
                "side": side,
                "pnl_pct": round(pnl_pct, 6),
                "current_price": current_price,
                "ema21": current_ema21,
            ...
            self.log_error("fast_reversal_eval", str(e))
            return {"action": "hold", "reason": f"fast_reversal_error:{e}"}

    def handle_fast_reversal(self, symbol: str, indicators: Dict[str, Any], current_price: float) -> bool:
        """Execute defensive action for a detected short-timeframe reversal."""
        try:
            evaluation = self.evaluate_fast_reversal(symbol, indicators, current_price)
            action = evaluation.get("action")
            if action == "hold":
                return False

            state = self._position_states.setdefault(symbol, {})
            partial_state = state.setdefault("partial_take_profit_state", {})
            if action == "partial":
                if partial_state.get("fast_reversal_partial_done"):
                    return False
                if self.close_partial_position(symbol, 0.50, "fast_reversal_partial"):
                    partial_state["fast_reversal_partial_done"] = {
                        "timestamp": int(datetime.now().timestamp() * 1000),
                        "evaluation": evaluation,
                    }
                    self.trading_results.setdefault("operational_events", []).append({
                        "timestamp": datetime.now().isoformat(),
                        "type": "fast_reversal_partial",
                        "symbol": symbol,
                        "evaluation": evaluation,
                    })
                    return True
            ...
            if action == "close":
                if self.close_position(symbol, "fast_reversal_close"):
                    self.trading_results.setdefault("operational_events", []).append({
                        "timestamp": datetime.now().isoformat(),
                        "type": "fast_reversal_close",
                        "symbol": symbol,
                        "evaluation": evaluation,
                    })
                    return True
            ...
            self.log_error("fast_reversal_handle", str(e))
            return False
    ...
    def manage_open_positions(self, market_data: Dict[str, Any], 
        ...
        try:
            ...
            for symbol in list(active_positions.keys()):
                ...
                # 1. Fast reversal defense: short timeframes can turn before
                # the 1h trend classifier catches up.
                if self.handle_fast_reversal(symbol, indicators.get(symbol, {}), current_price):
                    continue
