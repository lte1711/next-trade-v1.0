...
class MarketDataService:
    ...
    def __init__(self, base_url: str = "https://demo-fapi.binance.com"):
        ...
        self._symbol_name_pattern = re.compile(r"^[A-Z0-9]+USDT$")
    ...
    def update_market_data(self, symbols, intervals=['1m', '5m', '15m', '30m', '1h']):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for symbol_info in exchange_info['symbols']:
                    ...
                    if (
                        not symbol_name.isascii()
                        or not self._symbol_name_pattern.match(symbol_name)
                        or base_asset.isdigit()
                    ):
                        continue
                    ...
                    if (symbol_info['status'] == 'TRADING' and 
                        ...
                        ticker_data = ticker_lookup.get(symbol_name)
                        if not ticker_data:
                            continue
                        ...
                        trade_count = int(ticker_data.get('count', 0) or 0)
                        price_change_percent = abs(float(ticker_data.get('priceChangePercent', 0) or 0))
                        notional_volume = float(ticker_data.get('volume', 0)) * last_price
                ...
                # Sort by usable notional liquidity, not raw token count. This prevents
                # ultra-low-price symbols from dominating simply because base volume is huge.
                symbols.sort(
                    key=lambda x: (
                        x.get('quote_volume', 0.0),
                        x.get('notional_volume', 0.0),
                        x.get('trade_count', 0),
                        x.get('price_change_percent', 0.0)
                    ),
                    reverse=True
                )
                symbols = symbols[:50]
