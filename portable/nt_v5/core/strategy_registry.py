...
class StrategyRegistry:
    ...
    def select_preferred_symbols(
        ...
        try:
            ...
            default_modes = {
                "ma_trend_follow": "balanced",
                "ema_crossover": "volatile",
            }
            symbol_mode = symbol_selection.get(
                "symbol_mode",
                default_modes.get(strategy_name, "leaders"),
            )
            candidate_limit = min(
                max_symbols,
            ...
            if symbol_mode == "volatile":
                selected = symbols[1 : min(len(symbols), candidate_limit * 3) : 2]
                if not selected:
            ...
            if symbol_mode == "balanced":
                ...
                middle_start = max(0, len(symbols) // 3)
                middle = symbols[middle_start : middle_start + max(1, candidate_limit // 4)]
                tail = symbols[-max(1, candidate_limit // 4) :]
                deduped: List[str] = []
                seen = set()
                for symbol in head + middle + tail:
