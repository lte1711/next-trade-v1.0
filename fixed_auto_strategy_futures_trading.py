#!/usr/bin/env python3
"""
수정된 자동 전략 기반 선물 거래 시스템 - 데이터 수신 오류 수정
"""
...
class FixedAutoStrategyFuturesTrading:
    def __init__(self):
    ...
    def get_available_symbols(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"❌ 심볼 정보 가져오기 실패: {response.status_code}")
    ...
    def update_market_data(self):
        ...
        try:
            ...
            print(f"❌ 시장 데이터 업데이트 오류: {e}")
    
    def display_status(self):
        """상태 표시 (수정된 버전 - 실제 데이터 기반)"""
        ...
        if recent_trades:
            for trade in recent_trades:
                status_icon = "✅" if trade["status"] == "FILLED" else "⏳" if trade["status"] == "NEW" else "❌"
    ...
    def save_results(self):
        ...
        filename = f"fixed_auto_strategy_futures_trading_{timestamp}.json"
        ...
        print(f"📁 수정된 자동 전략 거래 결과 저장: {filename}")
    ...
    def run_trading(self):
        ...
        try:
            ...
            self.running = False
            self.save_results()
            print("🎉 수정된 자동 전략 기반 선물 거래 완료!")

if __name__ == "__main__":
    trading = FixedAutoStrategyFuturesTrading()
    trading.run_trading()
