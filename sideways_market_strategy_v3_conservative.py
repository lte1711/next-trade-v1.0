#!/usr/bin/env python3
"""
횡보장 전략 v3 백테스트 실행기 - 보수적 파라미터 버전
...
@dataclass
class RiskConfig:
    risk_per_trade: float = 0.001     # 0.1% of equity (더 감소)
    atr_stop_mult: float = 2.5        # 2.5x ATR (더 증가)
    atr_take_mult: float = 2.0        # 2.0x ATR (더 증가)
    fee_rate: float = 0.0004          # 0.04% per side
    slippage_rate: float = 0.0002     # 0.02% per side
    max_holding_bars: int = 8         # 8 bars (40분, 더 감소)
...
class StrategyConfig:
    ...
    rsi_period: int = 14
    bb_period: int = 20
    bb_std: float = 1.5              # 1.5 (더 감소)
    z_period: int = 20
    ranging_adx_threshold: float = 30.0  # 30.0 (더 증가)
    trending_adx_threshold: float = 35.0  # 35.0 (더 증가)
    bandwidth_quantile_window: int = 100
    bandwidth_ranging_quantile: float = 0.45  # 0.45 (더 증가)
    rsi_overbought: float = 60.0     # 60.0 (더 감소)
    rsi_oversold: float = 40.0        # 40.0 (더 증가)
    z_entry_threshold: float = 1.5      # 1.5 (더 감소)
    volume_lookback: int = 20
    low_volume_factor: float = 0.6     # 0.6 (더 증가)
    tr_spike_atr_mult: float = 2.5     # 2.5 (더 증가)
...
class SidewaysMarketStrategyV3:
    ...
    def _signal_row(self, row: pd.Series) -> pd.Series:
        ...
        if pd.notna(close) and pd.notna(lower) and pd.notna(rsi):
            if close <= lower and rsi <= self.cfg.rsi_oversold:
                ...
                rsi_score = min(1.0, max(0.0, (self.cfg.rsi_oversold - rsi) / 40.0))
                adx_score = 0.0 if pd.isna(adx) else min(1.0, max(0.0, (self.cfg.ranging_adx_threshold - adx) / 30.0))
                conf = self._confidence_clip(0.45 + 0.40 * rsi_score + 0.10 * distance_score + 0.05 * adx_score)
                return pd.Series(
                    {
                        "signal": "BUY",
                        "signal_source": "RANGE",
        ...
        if pd.notna(close) and pd.notna(upper) and pd.notna(rsi):
            if close >= upper and rsi >= self.cfg.rsi_overbought:
                ...
                rsi_score = min(1.0, max(0.0, (rsi - self.cfg.rsi_overbought) / 40.0))
                adx_score = 0.0 if pd.isna(adx) else min(1.0, max(0.0, (self.cfg.ranging_adx_threshold - adx) / 30.0))
                conf = self._confidence_clip(0.45 + 0.40 * rsi_score + 0.10 * distance_score + 0.05 * adx_score)
                return pd.Series(
                    {
        ...
        # Mean reversion fallback - 더 보수적
        if pd.notna(z):
            if z <= -self.cfg.z_entry_threshold:
                z_score = min(1.0, abs(z) / (self.cfg.z_entry_threshold + 1.0))
                pb_score = 0.0 if pd.isna(pb) else min(1.0, max(0.0, (0.1 - pb) / 0.1))
                conf = self._confidence_clip(0.40 + 0.55 * z_score + 0.05 * pb_score)
                return pd.Series(
                    {
                        "signal": "BUY",
            ...
            if z >= self.cfg.z_entry_threshold:
                z_score = min(1.0, abs(z) / (self.cfg.z_entry_threshold + 1.0))
                pb_score = 0.0 if pd.isna(pb) else min(1.0, max(0.0, (pb - 0.9) / 0.1))
                conf = self._confidence_clip(0.40 + 0.55 * z_score + 0.05 * pb_score)
                return pd.Series(
                    {
                        "signal": "SELL",
    ...
    def backtest(
        ...
        min_confidence: float = 0.75,  # 더 증가
...
# 메인 실행 함수
def main():
    """메인 실행 함수 - 보수적 파라미터로 백테스트"""
    print("🎯 횡보장 전략 v3 백테스트 시작 (보수적 파라미터)")
    print("=" * 60)
    ...
    output_dir = f"backtest_results_conservative_{timestamp}"
    ...
    print("🔄 성과 비교")
    print("-" * 50)
    print("이전 결과: -83.06% 수익률, 26.55% 승률")
    print("개선 결과: -88.21% 수익률, 29.01% 승률")
    print("현재 결과: {:.2f}% 수익률, {:.2f}% 승률".format(
