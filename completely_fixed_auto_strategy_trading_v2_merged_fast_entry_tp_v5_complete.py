...
class CompletelyFixedAutoStrategyFuturesTrading:
    def __init__(self):
        ...
        self.partial_tp1_pct = self.safe_float_conversion(os.getenv("PARTIAL_TP1_PCT"), 0.8)
        self.partial_tp2_pct = self.safe_float_conversion(os.getenv("PARTIAL_TP2_PCT"), 1.5)
        self.partial_tp1_close_ratio = min(max(self.safe_float_conversion(os.getenv("PARTIAL_TP1_CLOSE_RATIO"), 0.30), 0.05), 0.95)
        self.partial_tp2_close_ratio = min(max(self.safe_float_conversion(os.getenv("PARTIAL_TP2_CLOSE_RATIO"), 0.30), 0.05), 0.95)
        self.break_even_buffer_pct = self.safe_float_conversion(os.getenv("BREAK_EVEN_BUFFER_PCT"), 0.0) / 100.0
        self.trailing_activation_pct = self.safe_float_conversion(os.getenv("TRAILING_ACTIVATION_PCT"), 2.0)
        self.trailing_stop_buffer_pct = self.safe_float_conversion(os.getenv("TRAILING_STOP_BUFFER_PCT"), 0.8)
        self.minimum_candidate_score = self.safe_float_conversion(os.getenv("MINIMUM_CANDIDATE_SCORE"), 12.0)
        self.edge_weighting_enabled = os.getenv("EDGE_WEIGHTING_ENABLED", "true").lower() == "true"
        self.min_position_allocation_pct = min(max(self.safe_float_conversion(os.getenv("MIN_POSITION_ALLOCATION_PCT"), 5.0), 1.0), 100.0) / 100.0
        self.max_position_allocation_pct = min(max(self.safe_float_conversion(os.getenv("MAX_POSITION_ALLOCATION_PCT"), 20.0), 5.0), 100.0) / 100.0
        self.score_power = min(max(self.safe_float_conversion(os.getenv("SCORE_POWER"), 1.35), 0.5), 3.0)
        self.volatility_scaling_enabled = os.getenv("VOLATILITY_SCALING_ENABLED", "true").lower() == "true"
        self.low_volatility_bonus = min(max(self.safe_float_conversion(os.getenv("LOW_VOLATILITY_BONUS"), 1.15), 0.5), 2.0)
        self.high_volatility_penalty = min(max(self.safe_float_conversion(os.getenv("HIGH_VOLATILITY_PENALTY"), 0.75), 0.1), 1.5)
        self.risk_position_cap_pct = min(max(self.safe_float_conversion(os.getenv("RISK_POSITION_CAP_PCT"), 25.0), 1.0), 100.0) / 100.0
        self.max_total_allocation_pct = min(max(self.safe_float_conversion(os.getenv("MAX_TOTAL_ALLOCATION_PCT"), 80.0), 1.0), 100.0) / 100.0
        self.fast_entry_enabled = os.getenv("FAST_ENTRY_ENABLED", "true").lower() == "true"
        self.fast_entry_consensus_relief = int(self.safe_float_conversion(os.getenv("FAST_ENTRY_CONSENSUS_RELIEF"), 1))
        self.fast_entry_score_bonus = self.safe_float_conversion(os.getenv("FAST_ENTRY_SCORE_BONUS"), 1.5)
        self.fast_entry_require_1h_not_against = os.getenv("FAST_ENTRY_REQUIRE_1H_NOT_AGAINST", "true").lower() == "true"
        self.fast_entry_min_trend_consensus_abs = int(self.safe_float_conversion(os.getenv("FAST_ENTRY_MIN_TREND_CONSENSUS_ABS"), 2))
        self.fast_entry_min_quality_score = int(self.safe_float_conversion(os.getenv("FAST_ENTRY_MIN_QUALITY_SCORE"), 2))
        self.fast_entry_wait_confirm_bar = os.getenv("FAST_ENTRY_WAIT_CONFIRM_BAR", "true").lower() == "true"
        self.fast_entry_size_multiplier = min(max(self.safe_float_conversion(os.getenv("FAST_ENTRY_SIZE_MULTIPLIER"), 0.5), 0.1), 1.0)
        self.fast_tight_stop_loss_pct = self.safe_float_conversion(os.getenv("FAST_TIGHT_STOP_LOSS_PCT"), 0.3) / 100.0
        self.fast_tp1_pct = self.safe_float_conversion(os.getenv("FAST_TP1_PCT"), 0.5)
        self.fast_tp2_pct = self.safe_float_conversion(os.getenv("FAST_TP2_PCT"), 1.2)
        self.fast_tp1_close_ratio = min(max(self.safe_float_conversion(os.getenv("FAST_TP1_CLOSE_RATIO"), 0.50), 0.05), 0.95)
        self.fast_tp2_close_ratio = min(max(self.safe_float_conversion(os.getenv("FAST_TP2_CLOSE_RATIO"), 1.0), 0.05), 1.0)
        self.max_hold_minutes = int(self.safe_float_conversion(os.getenv("MAX_HOLD_MINUTES"), 30))
        self.fast_max_hold_minutes = int(self.safe_float_conversion(os.getenv("FAST_MAX_HOLD_MINUTES"), 20))
        self.force_close_before_funding = os.getenv("FORCE_CLOSE_BEFORE_FUNDING", "true").lower() == "true"
        self.funding_close_lead_minutes = int(self.safe_float_conversion(os.getenv("FUNDING_CLOSE_LEAD_MINUTES"), 10))
    ...
    def sync_positions(self):
        ...
        if hasattr(self, '_last_sync_time') and current_time - self._last_sync_time < 2:
            return
    ...
    def analyze_timeframe_ma(self, symbol, interval, limit=120):
        ...
        breakout_hold_up = bool(last_fractal_high is not None and recent_breakout_up and (not fractal_breakout_up) and current_price > last_fractal_high)
        breakout_hold_down = bool(last_fractal_low is not None and recent_breakout_down and (not fractal_breakout_down) and current_price < last_fractal_low)
        ...
        return {
            ...
            "breakout_hold_up": breakout_hold_up,
            "breakout_hold_down": breakout_hold_down,
    ...
    def get_ma_trade_decision(self, strategy, market_regime):
        ...
        if strict_long_ready and not strict_short_ready:
            market_regime["entry_mode"] = "STRICT_LONG"
            return "BUY"
        if strict_short_ready and not strict_long_ready:
            market_regime["entry_mode"] = "STRICT_SHORT"
            return "SELL"

        if not self.fast_entry_enabled:
            return None

        fast_consensus_threshold = max(0, consensus_threshold - self.fast_entry_consensus_relief)
        bullish_fast_ha_ready = bullish_ha_ready or (
            tf_15m.get("ha_trend") == "BULLISH" and
            (tf_5m.get("ha_trend") == "BULLISH" or tf_5m.get("ha_strong_bullish"))
        )
        bearish_fast_ha_ready = bearish_ha_ready or (
            tf_15m.get("ha_trend") == "BEARISH" and
            (tf_5m.get("ha_trend") == "BEARISH" or tf_5m.get("ha_strong_bearish"))
        )

        one_hour_not_bearish = tf_1h.get("alignment") != "BEARISH"
        one_hour_not_bullish = tf_1h.get("alignment") != "BULLISH"
        if not self.fast_entry_require_1h_not_against:
            one_hour_not_bearish = True
            one_hour_not_bullish = True

        fast_breakout_long_ready = any(
            timeframe_data.get(interval, {}).get("breakout_hold_up")
            for interval in fractal_intervals
        ) if self.fast_entry_wait_confirm_bar else bullish_breakout_context
        fast_breakout_short_ready = any(
            timeframe_data.get(interval, {}).get("breakout_hold_down")
            for interval in fractal_intervals
        ) if self.fast_entry_wait_confirm_bar else bearish_breakout_context

        fast_long_quality_score = 0
        if market_regime.get("trend_consensus", 0) >= max(fast_consensus_threshold, self.fast_entry_min_trend_consensus_abs):
            fast_long_quality_score += 1
        if volume_expansion_count >= 1:
            fast_long_quality_score += 1
        if tf_5m.get("ha_strong_bullish") or tf_15m.get("ha_trend") == "BULLISH":
            fast_long_quality_score += 1
        if tf_15m.get("price_vs_ma") == "ABOVE" or tf_15m.get("ema_fast", 0) > tf_15m.get("ema_mid", 0):
            fast_long_quality_score += 1

        fast_short_quality_score = 0
        if market_regime.get("trend_consensus", 0) <= -max(fast_consensus_threshold, self.fast_entry_min_trend_consensus_abs):
            fast_short_quality_score += 1
        if volume_expansion_count >= 1:
            fast_short_quality_score += 1
        if tf_5m.get("ha_strong_bearish") or tf_15m.get("ha_trend") == "BEARISH":
            fast_short_quality_score += 1
        if tf_15m.get("price_vs_ma") == "BELOW" or tf_15m.get("ema_fast", 0) < tf_15m.get("ema_mid", 0):
            fast_short_quality_score += 1

        fast_long_ready = (
            bullish_alignment_count >= max(1, required_alignment_count) and
            tf_5m.get("price_vs_ma") == "ABOVE" and
            tf_5m.get("ema_fast", 0) > tf_5m.get("ema_mid", 0) and
            (
                tf_15m.get("price_vs_ma") == "ABOVE" or
                tf_15m.get("ema_fast", 0) > tf_15m.get("ema_mid", 0)
            ) and
            market_regime.get("trend_consensus", 0) >= fast_consensus_threshold and
            bullish_fast_ha_ready and
            bullish_cross_ready and
            volume_ready and
            (fast_breakout_long_ready or bullish_pullback_ready) and
            one_hour_not_bearish and
            fast_long_quality_score >= self.fast_entry_min_quality_score
        )

        fast_short_ready = (
            bearish_alignment_count >= max(1, required_alignment_count) and
            tf_5m.get("price_vs_ma") == "BELOW" and
            tf_5m.get("ema_fast", 0) < tf_5m.get("ema_mid", 0) and
            (
                tf_15m.get("price_vs_ma") == "BELOW" or
                tf_15m.get("ema_fast", 0) < tf_15m.get("ema_mid", 0)
            ) and
            market_regime.get("trend_consensus", 0) <= -fast_consensus_threshold and
            bearish_fast_ha_ready and
            bearish_cross_ready and
            volume_ready and
            (fast_breakout_short_ready or bearish_pullback_ready) and
            one_hour_not_bullish and
            fast_short_quality_score >= self.fast_entry_min_quality_score
        )

        market_regime["fast_entry_debug"] = {
            "fast_consensus_threshold": fast_consensus_threshold,
            "fast_breakout_long_ready": fast_breakout_long_ready,
            "fast_breakout_short_ready": fast_breakout_short_ready,
            "fast_long_quality_score": fast_long_quality_score,
            "fast_short_quality_score": fast_short_quality_score,
            "bullish_pullback_ready": bullish_pullback_ready,
            "bearish_pullback_ready": bearish_pullback_ready,
            "one_hour_not_bearish": one_hour_not_bearish,
            "one_hour_not_bullish": one_hour_not_bullish,
        }

        if fast_long_ready and not fast_short_ready:
            market_regime["entry_mode"] = "FAST_LONG"
            return "BUY"
        if fast_short_ready and not fast_long_ready:
            market_regime["entry_mode"] = "FAST_SHORT"
            return "SELL"
        return None
    ...
    def score_trade_candidate(self, market_regime):
        ...
        entry_mode = market_regime.get("entry_mode")
        if entry_mode in {"FAST_LONG", "FAST_SHORT"}:
            score += self.fast_entry_score_bonus
    ...
    def place_protective_orders(self, strategy_name, symbol, entry_side, entry_price):
        ...
        session_name = self.get_market_session()
        session_policy = self.get_session_policy(session_name)
        last_entry_trade = self.get_last_entry_trade(symbol) or {}
        entry_mode = last_entry_trade.get("entry_mode")
        ...
        if entry_mode in {"FAST_LONG", "FAST_SHORT"} and self.fast_tight_stop_loss_pct > 0:
            stop_loss_pct = min(stop_loss_pct, self.fast_tight_stop_loss_pct)
    ...
    def get_position_profit_pct(self, position):
        """Return position profit percentage using live mark price and entry price."""
        amount = self.safe_float_conversion(position.get("amount"), 0.0)
        entry_price = self.safe_float_conversion(position.get("entry_price"), 0.0)
        mark_price = self.safe_float_conversion(position.get("mark_price"), 0.0)
        if amount == 0 or entry_price <= 0 or mark_price <= 0:
            return 0.0
        direction = 1 if amount > 0 else -1
        return ((mark_price - entry_price) / entry_price) * 100.0 * direction

    def get_position_entry_mode(self, symbol):
        """Return the recorded entry mode for the latest non-reduce-only filled/new trade."""
        trade = self.get_last_entry_trade(symbol) or {}
        return trade.get("entry_mode")

    def get_position_hold_seconds(self, symbol, position=None):
        """Return how long the position has been open in seconds."""
        entry_time_str = self.get_position_entry_time(symbol)
        if not entry_time_str and position:
            entry_time_str = position.get("entry_time")
        try:
            entry_time = datetime.fromisoformat(entry_time_str)
        except Exception:
            return 0.0
        return max(0.0, (datetime.now() - entry_time).total_seconds())
    ...
    def get_next_funding_time_utc(self, now=None):
        ...
        for hour in funding_hours:
            ...
            if candidate > now:
                return candidate
        return (aligned + timedelta(days=1)).replace(hour=0)

    def is_funding_close_window(self, now=None):
        """Return whether the current time is within the pre-funding close window."""
        if not self.force_close_before_funding:
            return False
        now = now or datetime.now(timezone.utc)
        next_funding = self.get_next_funding_time_utc(now)
        seconds_to_funding = (next_funding - now).total_seconds()
        return 0 <= seconds_to_funding <= (self.funding_close_lead_minutes * 60)

    def should_force_close_for_max_hold(self, symbol, position=None):
        """Return a reason string when the position exceeded the configured max hold window."""
        hold_seconds = self.get_position_hold_seconds(symbol, position)
        entry_mode = self.get_position_entry_mode(symbol)
        max_minutes = self.fast_max_hold_minutes if entry_mode in {"FAST_LONG", "FAST_SHORT"} else self.max_hold_minutes
        if max_minutes <= 0:
            return None
        if hold_seconds >= (max_minutes * 60):
            return "fast_max_hold_timeout" if entry_mode in {"FAST_LONG", "FAST_SHORT"} else "max_hold_timeout"
        return None

    def should_force_close_for_funding(self, symbol, position=None):
        """Return a reason string when the position should be closed before funding."""
        if not self.is_funding_close_window():
            return None
        hold_seconds = self.get_position_hold_seconds(symbol, position)
        if hold_seconds < 30:
            return None
        entry_mode = self.get_position_entry_mode(symbol)
        return "fast_pre_funding_close" if entry_mode in {"FAST_LONG", "FAST_SHORT"} else "pre_funding_close"

    def tighten_symbol_stop(self, symbol, position, stop_price, reason):
        """Move the protective stop only in a more favorable direction."""
        amount = self.safe_float_conversion(position.get("amount"), 0.0)
        entry_price = self.safe_float_conversion(position.get("entry_price"), 0.0)
        if amount == 0 or entry_price <= 0 or stop_price <= 0:
            return False

        previous_stop = self.safe_float_conversion(self.managed_stop_prices.get(symbol), 0.0)
        is_long = amount > 0
        if previous_stop > 0:
            if is_long and stop_price <= previous_stop:
                return False
            if (not is_long) and stop_price >= previous_stop:
                return False

        exit_side = "SELL" if is_long else "BUY"
        self.cancel_symbol_protective_orders(symbol)
        result = self.submit_protective_order(symbol, exit_side, "STOP_MARKET", stop_price)
        if result:
            self.managed_stop_prices[symbol] = stop_price
            print(f"[STOP_TIGHTEN] {symbol} | reason={reason} | stop={self.format_price_for_symbol(symbol, stop_price)}")
            return True
        return False

    def close_partial_position(self, symbol, position, close_ratio, reason):
        """Close part of an open position using a reduce-only market order."""
        amount = abs(self.safe_float_conversion(position.get("amount"), 0.0))
        if amount <= 0:
            return None
        close_ratio = min(max(self.safe_float_conversion(close_ratio, 0.0), 0.0), 1.0)
        if close_ratio <= 0:
            return None
        close_qty = amount * close_ratio
        if close_qty <= 0:
            return None
        ...
        return self.submit_order("position_manager", symbol, side, close_qty, reduce_only=True, metadata=metadata)

    def manage_profit_targets(self, symbol, position):
        """Apply staged profit-taking and stop tightening for profitable positions."""
        amount = self.safe_float_conversion(position.get("amount"), 0.0)
        entry_price = self.safe_float_conversion(position.get("entry_price"), 0.0)
        mark_price = self.safe_float_conversion(position.get("mark_price"), 0.0)
        if amount == 0 or entry_price <= 0 or mark_price <= 0:
        ...
        profit_pct = self.get_position_profit_pct(position)
        is_long = amount > 0
        entry_mode = self.get_position_entry_mode(symbol)
        is_fast_entry = entry_mode in {"FAST_LONG", "FAST_SHORT"}

        if is_fast_entry:
            if (not state.get("fast_tp1_done")) and profit_pct >= self.fast_tp1_pct:
                result = self.close_partial_position(symbol, position, self.fast_tp1_close_ratio, "fast_take_profit_1")
                if result:
                    state["fast_tp1_done"] = True
                    state["last_partial_close_at"] = datetime.now().isoformat()
                    break_even_stop = entry_price * (1 + self.break_even_buffer_pct if is_long else 1 - self.break_even_buffer_pct)
                    self.tighten_symbol_stop(symbol, position, break_even_stop, "fast_tp1_break_even")
                    return True

            if (not state.get("fast_tp2_done")) and profit_pct >= self.fast_tp2_pct:
                result = self.close_partial_position(symbol, position, self.fast_tp2_close_ratio, "fast_take_profit_2")
                if result:
                    state["fast_tp2_done"] = True
                    state["last_partial_close_at"] = datetime.now().isoformat()
                    locked_profit_stop = entry_price * (1.002 if is_long else 0.998)
                    self.tighten_symbol_stop(symbol, position, locked_profit_stop, "fast_tp2_lock_profit")
                    return True

        if (not state.get("tp1_done")) and profit_pct >= self.partial_tp1_pct:
            result = self.close_partial_position(symbol, position, self.partial_tp1_close_ratio, "partial_take_profit_1")
            if result:
                state["tp1_done"] = True
                state["last_partial_close_at"] = datetime.now().isoformat()
                break_even_stop = entry_price * (1 + self.break_even_buffer_pct if is_long else 1 - self.break_even_buffer_pct)
                self.tighten_symbol_stop(symbol, position, break_even_stop, "tp1_break_even")
                return True

        if (not state.get("tp2_done")) and profit_pct >= self.partial_tp2_pct:
            result = self.close_partial_position(symbol, position, self.partial_tp2_close_ratio, "partial_take_profit_2")
            if result:
                state["tp2_done"] = True
                state["last_partial_close_at"] = datetime.now().isoformat()
                locked_profit_stop = entry_price * (1.01 if is_long else 0.99)
                self.tighten_symbol_stop(symbol, position, locked_profit_stop, "tp2_lock_profit")
                return True

        if profit_pct >= self.trailing_activation_pct:
            if is_long:
                candidate_stop = mark_price * (1 - self.trailing_stop_buffer_pct / 100.0)
                candidate_stop = max(candidate_stop, entry_price * (1 + self.break_even_buffer_pct))
            else:
                candidate_stop = mark_price * (1 + self.trailing_stop_buffer_pct / 100.0)
                candidate_stop = min(candidate_stop, entry_price * (1 - self.break_even_buffer_pct))
            return self.tighten_symbol_stop(symbol, position, candidate_stop, "dynamic_trailing")
        ...
        for trade in self.trading_results.get("real_orders", []):
            ...
            if trade.get("reduce_only"):
                if trade["status"] == "FILLED":
                    ...
                    # self.update_entry_mode_performance(symbol, trade.get("realized_pnl"))

                    # 포지션 동기화 후 완전청산 여부 확인
                    self.sync_positions()
        ...
        try:
            ...
            if response is not None and response.status_code == 200:
                ...
                if reduce_only and status == "FILLED":
                    ...
                    # self.update_entry_mode_performance(symbol, trade_record.get("realized_pnl"))
                    ...
                    self.get_position_management_state(symbol)
    ...
    def execute_strategy_trade(self, strategy_name):
        ...
        try:
                    ...
                    try:
                        ...
                        for tf_name, tf_data in market_regime.get("timeframes", {}).items():
                            diagnostic_data["market_conditions"][symbol]["timeframes"][tf_name] = {
                                ...
                                "breakout_hold_up": tf_data.get("breakout_hold_up"),
            ...
            if self.safe_float_conversion(best_candidate.get("score"), 0.0) < self.minimum_candidate_score:
                self.emit_signal_diagnostic(strategy_name, "score_below_threshold", candidate_evaluations)
                return None
            self.trading_results["market_regime"] = best_candidate["market_regime"]["regime"]
            selected_entry_mode = best_candidate["market_regime"].get("entry_mode")
            quantity, allocation_context = self.calculate_position_size(
                strategy_name,
                best_candidate["symbol"],
                selected_entry_mode,
                candidate_score=best_candidate.get("score"),
                candidate_scores=[item.get("score") for item in candidate_evaluations],
            ...
            return self.submit_order(
                ...
                metadata={
                    ...
                    "candidate_score": round(self.safe_float_conversion(best_candidate.get("score"), 0.0), 6),
                    "selected_regime": best_candidate["market_regime"].get("regime"),
                    "trend_consensus": best_candidate["market_regime"].get("trend_consensus"),
                    "entry_mode": selected_entry_mode,
                    "position_size_multiplier_applied": allocation_context.get("position_size_multiplier_applied"),
                    "allocation_fraction": allocation_context.get("allocation_fraction"),
                    "risk_amount": allocation_context.get("risk_amount"),
                    "stop_loss_pct_used": allocation_context.get("stop_loss_pct_used"),
                    "volatility_multiplier": allocation_context.get("volatility_multiplier"),
                    "fast_tp1_pct": self.fast_tp1_pct if selected_entry_mode in {"FAST_LONG", "FAST_SHORT"} else None,
                    "fast_tp2_pct": self.fast_tp2_pct if selected_entry_mode in {"FAST_LONG", "FAST_SHORT"} else None,
                    "fast_entry_debug": best_candidate["market_regime"].get("fast_entry_debug"),
                },
            )
    ...
    def get_current_total_allocation_fraction(self):
        """Return the current approximate fraction of capital already committed to open positions."""
        total_capital = max(self.safe_float_conversion(self.total_capital, 0.0), 0.0)
        if total_capital <= 0:
            return 0.0

        total_notional = 0.0
        for position in self.trading_results.get("active_positions", {}).values():
            amount = abs(self.safe_float_conversion(position.get("amount"), 0.0))
            mark_price = self.safe_float_conversion(position.get("mark_price"), 0.0)
            entry_price = self.safe_float_conversion(position.get("entry_price"), 0.0)
            price = mark_price if mark_price > 0 else entry_price
            total_notional += amount * price
        return max(total_notional / total_capital, 0.0)

    def estimate_stop_loss_pct(self, strategy_name, entry_mode=None, market_regime=None):
        """Estimate stop-loss distance used for risk-based sizing."""
        strategy = self.strategies[strategy_name]
        stop_loss_pct = self.safe_float_conversion(strategy.get("stop_loss"), self.default_stop_loss_pct / 100.0)
        if entry_mode in {"FAST_LONG", "FAST_SHORT"}:
            stop_loss_pct = max(self.fast_tight_stop_loss_pct, 0.0005)

        market_regime = market_regime or {}
        strength = self.safe_float_conversion(market_regime.get("strength"), 0.0)
        consensus = abs(self.safe_float_conversion(market_regime.get("trend_consensus"), 0.0))
        if strength >= 0.75 and consensus >= 2:
            stop_loss_pct *= 0.9
        elif strength <= 0.45:
            stop_loss_pct *= 1.1
        return min(max(stop_loss_pct, 0.0005), 0.05)

    def calculate_volatility_size_multiplier(self, symbol, market_regime=None):
        """Scale size down in high volatility and slightly up in calmer conditions."""
        ...
        timeframe_data = market_regime.get("timeframes", {})
        tf_5m = timeframe_data.get("5m", {})
        recent_high = self.safe_float_conversion(tf_5m.get("recent_high"), 0.0)
        recent_low = self.safe_float_conversion(tf_5m.get("recent_low"), 0.0)
        mark_price = self.get_current_price(symbol, 0.0)
        if mark_price > 0 and recent_high > 0 and recent_low > 0 and recent_high >= recent_low:
            normalized_range = (recent_high - recent_low) / mark_price

        if normalized_range is None:
            klines = self.get_klines(symbol, "5m", 24)
            if klines:
                highs = [self.safe_float_conversion(row[2], 0.0) for row in klines]
                lows = [self.safe_float_conversion(row[3], 0.0) for row in klines]
                closes = [self.safe_float_conversion(row[4], 0.0) for row in klines]
                if highs and lows and closes and closes[-1] > 0:
                    normalized_range = (max(highs) - min(lows)) / closes[-1]

        if normalized_range is None:
            return 1.0
        if normalized_range <= 0.012:
            return self.low_volatility_bonus
        if normalized_range >= 0.035:
            return self.high_volatility_penalty
        return 1.0

    def calculate_allocation_fraction(self, strategy_name, candidate_score, candidate_scores=None, market_regime=None, entry_mode=None):
        """Allocate more capital to stronger candidates while respecting global caps."""
        strategy = self.strategies[strategy_name]
        base_fraction = self.safe_float_conversion(strategy.get("capital"), 0.0) / max(self.safe_float_conversion(self.total_capital, 0.0), 1e-9)
        base_fraction = max(min(base_fraction, self.max_position_allocation_pct), self.min_position_allocation_pct)

        if not self.edge_weighting_enabled:
            allocation_fraction = base_fraction
        else:
            valid_scores = [max(self.safe_float_conversion(score, 0.0), 0.0) for score in (candidate_scores or [])]
            candidate_score = max(self.safe_float_conversion(candidate_score, 0.0), 0.0)
            if valid_scores:
                weighted_scores = [max(score, 0.0001) ** self.score_power for score in valid_scores]
                numerator = max(candidate_score, 0.0001) ** self.score_power
                relative_share = numerator / max(sum(weighted_scores), 0.0001)
                allocation_fraction = max(self.min_position_allocation_pct, relative_share)
            else:
                allocation_fraction = base_fraction

            strength = self.safe_float_conversion((market_regime or {}).get("strength"), 0.0)
            consensus = abs(self.safe_float_conversion((market_regime or {}).get("trend_consensus"), 0.0))
            if strength >= 0.80 and consensus >= 2:
                allocation_fraction *= 1.20
            elif strength <= 0.45:
                allocation_fraction *= 0.85

        if entry_mode in {"FAST_LONG", "FAST_SHORT"}:
            allocation_fraction *= self.fast_entry_size_multiplier

        current_allocated = self.get_current_total_allocation_fraction()
        remaining_capacity = max(self.max_total_allocation_pct - current_allocated, 0.0)
        allocation_fraction = min(allocation_fraction, self.max_position_allocation_pct, remaining_capacity)
        return max(allocation_fraction, 0.0)

    def record_capital_allocation_decision(self, strategy_name, symbol, allocation_context):
        """Persist recent capital allocation decisions for audit and tuning."""
        decisions = self.trading_results.setdefault("capital_allocation_decisions", [])
        decision = {
            "timestamp": datetime.now().isoformat(),
            "strategy": strategy_name,
            "symbol": symbol,
        }
        decision.update(allocation_context)
        decisions.append(decision)
        if len(decisions) > 200:
            del decisions[:-200]

    def calculate_position_size(self, strategy_name, symbol, entry_mode=None, candidate_score=None, candidate_scores=None, market_regime=None):
        """Calculate a risk-based position size with edge-weighted capital allocation."""
        try:
            strategy = self.strategies[strategy_name]
            current_price = self.get_current_price(symbol, 0.0)
            if current_price <= 0:
            ...
            total_capital = self.safe_float_conversion(self.total_capital, 0.0)
            available_balance = self.safe_float_conversion(
                self.trading_results.get("available_balance"),
                total_capital
            )
            stop_loss_pct = self.estimate_stop_loss_pct(strategy_name, entry_mode=entry_mode, market_regime=market_regime)
            volatility_multiplier = self.calculate_volatility_size_multiplier(symbol, market_regime=market_regime)
            leverage = max(self.safe_float_conversion(strategy.get("leverage"), 1.0), 1.0)

            capital_slice = min(total_capital * allocation_fraction, available_balance * 0.95)
            risk_budget = capital_slice * max(self.safe_float_conversion(strategy.get("risk_per_trade"), 0.0), 0.0001)
            raw_notional = (risk_budget / max(stop_loss_pct, 0.0001)) * session_size_multiplier * volatility_multiplier
            max_notional_from_cap = capital_slice * leverage
            position_cap_notional = total_capital * self.risk_position_cap_pct
            position_value = min(raw_notional, max_notional_from_cap, position_cap_notional)
            if position_value <= 0:
                return 0.0, {}
            ...
            if symbol in min_quantities:
                quantity = max(quantity, min_quantities[symbol])

            allocation_context = {
                "allocation_fraction": round(allocation_fraction, 6),
                "risk_amount": round(risk_budget, 6),
                "stop_loss_pct_used": round(stop_loss_pct * 100.0, 6),
                "volatility_multiplier": round(volatility_multiplier, 6),
                "position_size_multiplier_applied": round(session_size_multiplier * volatility_multiplier, 6),
                "candidate_score": round(self.safe_float_conversion(candidate_score, 0.0), 6),
                "position_value": round(position_value, 6),
            }
            self.record_capital_allocation_decision(strategy_name, symbol, allocation_context)
            return quantity, allocation_context
    ...
    def close_position(self, symbol, position, reason):
        ...
        bypass_hold_reasons = {
            ...
            "max_hold_timeout",
            "fast_max_hold_timeout",
            "pre_funding_close",
            "fast_pre_funding_close"
        }
    ...
    def manage_open_positions(self):
        """Manage open positions, profit targets, funding windows, and exit signals."""
        try:
            self.sync_positions()
            active_positions = self.trading_results.get("active_positions", {})
            ...
            for symbol, position in list(active_positions.items()):
                forced_reason = self.should_force_close_for_funding(symbol, position) or self.should_force_close_for_max_hold(symbol, position)
                if forced_reason:
                    print(f"[FORCED_EXIT] {symbol} | reason={forced_reason}")
                    self.close_position(symbol, position, forced_reason)
                    continue

                if self.manage_profit_targets(symbol, position):
                    continue
    ...
    def display_status(self):
        ...
        print(f"[INFO] STRUCTURAL_NOTE: Current system uses monolithic architecture (2627 lines)")
        print(f"[INFO] STRUCTURAL_NOTE: Consider refactoring into separate modules for maintainability")
        print(f"[INFO] STRUCTURAL_NOTE: Key refactoring targets: submit_order, refresh_pending_orders, protective_orders")
        return None
...
# === MERGED FROM fast_entry_with_tp_v3.py ===
# These helpers mirror the standalone FAST TP example and delegate to the
# integrated class configuration when used with a plain position dict.

def apply_fast_tp(position, tp1_pct=0.5, tp2_pct=1.2):
    """
    Standalone FAST take-profit helper merged from fast_entry_with_tp_v3.py.
...
def apply_fast_risk_management(position, stop_loss_pct=0.3, tp1_pct=0.5, tp2_pct=1.2):
    """
    Standalone FAST risk helper merged from fast_entry_with_tp_v3.py.

    Returns:
        STOP_LOSS / CLOSE_PARTIAL / CLOSE_FULL / HOLD
    """
    pnl_pct = position.get("pnl_pct", 0)
    ...
    return apply_fast_tp(position, tp1_pct=tp1_pct, tp2_pct=tp2_pct)
