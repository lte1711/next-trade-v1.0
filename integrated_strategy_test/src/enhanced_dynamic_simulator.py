"""
개선된 동적 투자 시뮬레이터 v2.0
- 교체 기준: -2% → -1%로 조정
- 심볼 수: 5개 → 7-10개로 확장
- 시장 테스트: 변동성 높은 시장 추가 검증
"""
...
class EnhancedDynamicInvestmentSimulator:
    """개선된 동적 투자 시뮬레이터 v2.0"""
    
    def __init__(self, symbol_count: int = 8, replacement_threshold: float = -1.0):
        self.total_capital = 500.0  # 총 자본 $500
        self.initial_capital = 500.0
        self.current_capital = 500.0
        self.max_symbols = symbol_count  # 7-10개 심볼로 확장 (기본값 8개)
        ...
        # 개선된 동적 교체 파라미터
        self.performance_threshold = replacement_threshold  # -1% 이하면 교체 (개선)
        self.min_replacement_interval = 8  # 최소 8분 간격 교체 (단축)
        self.max_replacements_per_hour = 4  # 시간당 최대 4회 교체 (확장)
        self.last_replacement_time = None
        self.replacements_this_hour = 0
        self.last_hour_reset = datetime.now().hour
        
        # 변동성 측정 파라미터
        self.volatility_threshold = 3.0  # 변동성 3% 이상이면 고변동성 시장
        self.market_regime = "NORMAL"  # NORMAL, HIGH_VOLATILITY, EXTREME
        
        print(f"FACT: 개선된 동적 투자 시뮬레이터 v2.0 초기화")
        print(f"  💰 초기 자본: ${self.total_capital:.2f}")
        print(f"  🎯 투자 전략: {self.max_symbols}개 심볼 + 개선된 동적 교체")
        print(f"  🔄 교체 기준: {self.performance_threshold}% 이하 (개선)")
        print(f"  📊 대상 심볼: 거래량 기준 상위 80개")
        print(f"  🌊 변동성 분석: 자동 시장 상태 감지")
    ...
    def run_enhanced_dynamic_simulation(self, duration_minutes: int = 360, update_interval: int = 8):
        """개선된 동적 투자 시뮬레이션 실행"""
        print(f"FACT: 개선된 동적 투자 시뮬레이션 v2.0 시작")
        print(f"  💰 초기 자본: ${self.total_capital:.2f}")
        print(f"  🎯 투자 전략: {self.max_symbols}개 심볼 + 개선된 동적 교체")
...
def main():
    """메인 실행 함수"""
    print("🎯 개선된 동적 투자 검증 시뮬레이터 v2.0")
    print("개선 사항:")
    print("  🔄 교체 기준: -2% → -1%로 조정")
    print("  🎯 심볼 수: 5개 → 8개로 확장")
    print("  🌊 시장 테스트: 변동성 높은 시장 추가 검증")
    print()
    
    # 개선된 시뮬레이터 생성
    simulator = EnhancedDynamicInvestmentSimulator(
        symbol_count=8,  # 8개 심볼로 확장
        replacement_threshold=-1.0  # -1%로 조정
    )
    
    # 30분 테스트 시뮬레이션 실행
    result = simulator.run_enhanced_dynamic_simulation(duration_minutes=30, update_interval=5)
    
    if "error" not in result:
        # 최종 결과 출력
        print("\n" + "=" * 80)
        print("🎯 최종 개선된 동적 투자 시뮬레이션 결과")
        print("=" * 80)
        ...
        # 결과 저장
        results_file = Path("enhanced_dynamic_investment_simulation_results.json")
