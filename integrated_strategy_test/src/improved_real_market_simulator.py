...
class ImprovedRealMarketDynamicInvestmentSimulator:
    ...
    def __init__(self, symbol_count: int = 10, replacement_threshold: float = -0.8):
        ...
        # 수익 가능성 평가
        self.profitability_threshold = 60.0  # 수익 가능성 점수 기준
        self.min_profit_symbols = 3  # 최소 투자 심볼 수
        self.max_profit_symbols = 8  # 최대 투자 심볼 수
    ...
    def evaluate_profitability_potential(self, symbols: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """수익 가능성 평가"""
        print(f"FACT: 수익 가능성 평가 시작 (최소 {self.profitability_threshold}점)")
        ...
        for symbol_data in symbols:
            ...
            # 수익 가능성 평가
            if bullish_score >= self.profitability_threshold:
                # 추가 수익 가능성 평가
                profit_potential = self._calculate_profit_potential(symbol_data)
                
                if profit_potential >= 50:  # 수익 가능성 50% 이상
        ...
        print(f"  📊 수익 가능성 있는 심볼: {len(profitable_symbols)}개")
    ...
    def _calculate_profit_potential(self, symbol_data: Dict[str, Any]) -> float:
        ...
        # 상승 가능성 점수 (50%)
        bullish_score = symbol_data.get("bullish_score", 0)
        potential += bullish_score * 0.5
        
        # 현재 가격 변화 (20%)
        change_percent = symbol_data.get("change_percent", 0)
        if change_percent > 0:
            potential += min(change_percent * 2, 20)
        
        # 거래량 (15%)
        volume = symbol_data.get("volume", 0)
        volume_score = min(volume / 1000000, 15)
    ...
    def select_optimal_symbols(self, profitable_symbols: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """최적 심볼 선택"""
        print(f"FACT: 최적 심볼 선택 시작")
        
        # 현재 시장 상태에 따른 최대 심볼 수 조정
        if self.market_regime == "EXTREME":
            max_symbols = min(self.max_profit_symbols, 5)
        elif self.market_regime == "HIGH_VOLATILITY":
            max_symbols = min(self.max_profit_symbols, 6)
        else:
            max_symbols = self.max_profit_symbols
        
        # 수익 가능성이 높은 심볼 선택
        selected_symbols = profitable_symbols[:max_symbols]
        
        # 최소 심볼 수 확인
        if len(selected_symbols) < self.min_profit_symbols:
            print(f"  ⚠️ 수익 가능성 있는 심볼이 {self.min_profit_symbols}개 미만")
            return []
        
        print(f"  ✅ {len(selected_symbols)}개 심볼 선택 완료 (최대 {max_symbols}개)")
    ...
    def replace_underperforming_symbol_improved(self):
        ...
        available_symbols = [
            ...
            if s["symbol"] not in self.investments and s.get("profit_potential", 0) >= 50
        ]
        ...
        replacement_record = {
            ...
            "new_profit_potential": best_replacement.get("profit_potential", 0),
    ...
    def allocate_capital_profit_based(self, selected_symbols: List[Dict[str, Any]]) -> Dict[str, Any]:
        """수익 기반 자본 분배"""
        print(f"FACT: 수익 기반 자본 분배 시작")
        
        # 수익 가능성에 따른 가중치 계산
        total_potential = sum(s.get("profit_potential", 50) for s in selected_symbols)
        
        allocations = {}
        equal_allocation = self.total_capital / len(selected_symbols)
        ...
        for symbol_data in selected_symbols:
            ...
            # 수익 가능성에 따른 가중치
            weight = profit_potential / total_potential
            weighted_allocation = self.total_capital * weight
            
            # 최소/최대 할당 제한
            allocation = max(equal_allocation * 0.8, min(weighted_allocation, equal_allocation * 1.2))
        ...
        print(f"  ✅ {len(allocations)}개 심볼에 ${self.total_capital:.2f} 분배 완료")
        print(f"  📊 평균 할당: ${equal_allocation:.2f}")
    ...
    def run_improved_real_market_simulation(self, duration_minutes: int = 720, update_interval: int = 5):
        ...
        # 5. 자본 분배
        allocations = self.allocate_capital_profit_based(selected_symbols)
...
def main():
    ...
    # 12시간 시뮬레이션 실행
    result = simulator.run_improved_real_market_simulation(duration_minutes=720, update_interval=5)
