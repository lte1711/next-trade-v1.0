"""
추가 개선된 동적 투자 시뮬레이터 v3.0
- 교체 기준: -2% → -1% → -0.8%로 조정
- 심볼 수: 5개 → 8개 → 10개로 확장
- 시장 테스트: 고변동성 시나리오 적극 테스트
"""
...
class AdvancedDynamicInvestmentSimulator:
    """추가 개선된 동적 투자 시뮬레이터 v3.0"""
    ...
    def __init__(self, symbol_count: int = 10, replacement_threshold: float = -0.8):
        ...
        self.max_symbols = symbol_count  # 10개 심볼로 확장
        ...
        # 추가 개선된 동적 교체 파라미터
        self.performance_threshold = replacement_threshold  # -0.8% 이하면 교체
        self.min_replacement_interval = 5  # 최소 5분 간격 교체
        self.max_replacements_per_hour = 5  # 시간당 최대 5회 교체
        self.last_replacement_time = None
        self.replacements_this_hour = 0
        self.last_hour_reset = datetime.now().hour
        
        # 고변동성 시장 테스트 파라미터
        self.volatility_threshold = 2.5  # 변동성 2.5% 이상이면 고변동성
        self.extreme_volatility_threshold = 5.0  # 5% 이상이면 극단적
        self.market_regime = "NORMAL"  # NORMAL, HIGH_VOLATILITY, EXTREME
        self.simulated_volatility = False  # 시뮬레이션 모드
        
        # 테스트 단계별 설정
        self.test_phase = 1  # 1: 기본, 2: 정상, 3: 고변동성, 4: 극단적
        self.phase_start_time = datetime.now()
        
        print(f"FACT: 추가 개선된 동적 투자 시뮬레이터 v3.0 초기화")
        print(f"  💰 초기 자본: ${self.total_capital:.2f}")
        print(f"  🎯 투자 전략: {self.max_symbols}개 심볼 + 추가 개선된 동적 교체")
        print(f"  🔄 교체 기준: {self.performance_threshold}% 이하 (추가 개선)")
        print(f"  📊 대상 심볼: 거래량 기준 상위 80개")
        print(f"  🌊 고변동성 테스트: 시나리오별 자동 적용")
    ...
    def analyze_market_volatility(self) -> str:
        """시장 변동성 분석 (테스트 단계별 시뮬레이션 포함)"""
        if self.simulated_volatility:
            # 테스트 단계별 시장 상태 시뮬레이션
            ...
            if self.test_phase == 1:
                ...
                # 3단계: 고변동성 시뮬레이션 (대부분 고변동성)
                if elapsed_minutes < 2:
                    self.market_regime = "NORMAL"
                else:
                    # 70% 확률로 고변동성, 20% 확률로 극단적
                    rand = random.random()
                    if rand < 0.7:
                        self.market_regime = "HIGH_VOLATILITY"
                    elif rand < 0.9:
                ...
                # 4단계: 극단적 시장 테스트 (대부분 극단적)
                if elapsed_minutes < 1:
                    self.market_regime = "HIGH_VOLATILITY"
                else:
                    # 80% 확률로 극단적, 20% 확률로 고변동성
    ...
    def update_investment_performance(self):
        ...
        for symbol, investment in self.investments.items():
            try:
                # 현재 가격 조회 (시뮬레이션 모드)
                if self.simulated_volatility and self.market_regime != "NORMAL":
                    # 시뮬레이션된 가격 변동 적용
                    base_price = investment["current_price"]
                    
                    if self.market_regime == "HIGH_VOLATILITY":
                        # 고변동성: ±2% 변동
                        price_change = random.uniform(-0.02, 0.02)
                    elif self.market_regime == "EXTREME":
                        # 극단적: ±5% 변동
        ...
        performance_record = {
            ...
            "simulated_volatility": self.simulated_volatility,
    ...
    def should_replace_symbol(self, symbol: str) -> bool:
        ...
        if self.market_regime == "EXTREME":
            threshold = threshold * 0.5  # 극단적 시장에서는 더 빠른 교체
        elif self.market_regime == "HIGH_VOLATILITY":
            threshold = threshold * 0.75  # 고변동성 시장에서는 빠른 교체
    ...
    def replace_underperforming_symbol(self):
        """추가 개선된 부진 심볼 교체"""
        print(f"FACT: 추가 개선된 부진 심볼 교체 프로세스 시작")
        print(f"  🌊 현재 시장 상태: {self.market_regime}")
        print(f"  📋 테스트 단계: {self.test_phase}단계")
        ...
        # 시장 상태에 따른 심볼 선택 전략
        if self.market_regime == "EXTREME":
            # 극단적 시장에서는 가장 안정적인 심볼 선택
            best_replacement = min(available_symbols, key=lambda x: abs(x["change_percent"]))
        else:
            # 일반 시장에서는 가장 높은 상승 가능성 점수 심볼 선택
        ...
        replacement_record = {
            ...
            "market_regime": self.market_regime,
            "test_phase": self.test_phase,
            "replacement_threshold": threshold if 'threshold' in locals() else self.performance_threshold,
            "simulated": self.simulated_volatility
        }
    ...
    def run_advanced_dynamic_simulation(self, duration_minutes: int = 30, update_interval: int = 5):
        """추가 개선된 동적 투자 시뮬레이션 실행"""
        print(f"FACT: 추가 개선된 동적 투자 시뮬레이션 v3.0 시작")
        print(f"  💰 초기 자본: ${self.total_capital:.2f}")
        print(f"  🎯 투자 전략: {self.max_symbols}개 심볼 + 추가 개선된 동적 교체")
        ...
        while datetime.now() < end_time and iteration < max_iterations:
            ...
            if iteration == 1:
                self.test_phase = 1
                self.simulated_volatility = False
                self.phase_start_time = current_time
                print(f"\n📅 1단계: 기본 설정 시작 (0-5분)")
            elif iteration == 2:
                self.test_phase = 2
                self.simulated_volatility = True
                self.phase_start_time = current_time
                print(f"\n📅 2단계: 정상 시장 테스트 시작 (5-15분)")
            elif iteration == 4:
                self.test_phase = 3
                self.simulated_volatility = True
                self.phase_start_time = current_time
                print(f"\n📅 3단계: 고변동성 시뮬레이션 시작 (15-25분)")
            elif iteration == 6:
                self.test_phase = 4
                self.simulated_volatility = True
                self.phase_start_time = current_time
                print(f"\n📅 4단계: 극단적 시장 테스트 시작 (25-30분)")
    ...
    def _generate_final_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "test_phase_history": [p.get("test_phase", 1) for p in self.performance_history]
        }

def main():
    """메인 실행 함수"""
    print("🎯 추가 개선된 동적 투자 검증 시뮬레이터 v3.0")
    print("추가 개선 사항:")
    print("  🔄 교체 기준: -2% → -1% → -0.8%로 추가 조정")
    print("  🎯 심볼 수: 5개 → 8개 → 10개로 추가 확장")
    print("  📊 시장 테스트: 고변동성 시나리오 적극 테스트")
    print()
    
    # 추가 개선된 시뮬레이터 생성
    simulator = AdvancedDynamicInvestmentSimulator(
        symbol_count=10,  # 10개 심볼로 추가 확장
        replacement_threshold=-0.8  # -0.8%로 추가 조정
    )
    
    # 1시간 시장 테스트 시뮬레이션 실행
    result = simulator.run_advanced_dynamic_simulation(duration_minutes=60, update_interval=5)
    
    if "error" not in result:
        # 최종 결과 출력
        print("\n" + "=" * 80)
        print("🎯 최종 추가 개선된 동적 투자 시뮬레이션 결과")
        print("=" * 80)
        ...
        print(f"  📊 시뮬레이션 모드: {metadata['simulated_volatility']}")
        ...
        if result["replacement_history"]:
            ...
            for i, replacement in enumerate(result["replacement_history"], 1):
                ...
                print(f"     테스트 단계: {replacement['test_phase']}단계")
                print(f"     시뮬레이션: {'예' if replacement['simulated'] else '아니오'}")
        ...
        # 테스트 단계별 분석
        test_phases = result["test_phase_history"]
        phase_changes = {}
        for phase in test_phases:
            phase_changes[phase] = phase_changes.get(phase, 0) + 1
        
        print(f"\n📋 테스트 단계별 분석:")
        for phase, count in phase_changes.items():
            percentage = (count / len(test_phases)) * 100
            print(f"  {phase}단계: {count}회 ({percentage:.1f}%)")
        
        # 결과 저장
        results_file = Path("advanced_dynamic_investment_simulation_results.json")
