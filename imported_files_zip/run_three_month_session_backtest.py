"""Run the imported strategy and split performance by entry session.

The imported strategy modules are used as-is. This runner only groups the
resulting trades by UTC market session so we can evaluate when the strategy
works best.
"""
...
def summarize_trades(trades: list) -> dict:
    ...
    wins = [trade for trade in final_trades if trade.pnl_pct > 0]
    losses = [trade for trade in final_trades if trade.pnl_pct < 0]
    gross_win = sum(trade.pnl_pct for trade in final_trades if trade.pnl_pct > 0)
    gross_loss = abs(sum(trade.pnl_pct for trade in final_trades if trade.pnl_pct < 0))
    total_pnl = sum(trade.pnl_pct for trade in trades)
    by_signal = defaultdict(float)
    by_exit = Counter()
    by_side = Counter()
    for trade in trades:
        by_signal[clean_text(trade.signal.signal_type)] += float(trade.pnl_pct)
    ...
    return {
        ...
        "win_rate_pct": round((len(wins) / len(final_trades) * 100.0) if final_trades else 0.0, 6),
        "net_pnl_pct": round(float(total_pnl), 6),
        "avg_final_pnl_pct": round(float(sum(t.pnl_pct for t in final_trades) / len(final_trades)), 6) if final_trades else 0.0,
        "profit_factor": round(float(gross_win / gross_loss), 6) if gross_loss else None,
        "by_signal_type_pnl_pct": {key: round(value, 6) for key, value in sorted(by_signal.items())},
...
def merge_session_reports(symbol_reports: list[dict]) -> dict:
    ...
    for session in ["US_PEAK", "EU_PEAK", "ASIA_PEAK", "NORMAL", "DEAD_ZONE"]:
        ...
        for report in symbol_reports:
            ...
            for key in totals:
                totals[key] += item[key]
        totals["win_rate_pct"] = round((totals["wins"] / totals["final_trades"] * 100.0) if totals["final_trades"] else 0.0, 6)
        totals["net_pnl_pct"] = round(float(totals["net_pnl_pct"]), 6)
...
def main() -> None:
    ...
    output_path = OUTPUT_DIR / f"imported_strategy_3m_sessions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
