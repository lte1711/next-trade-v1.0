#!/usr/bin/env python3
"""
횡보장 전략 v3 백테스트 실행기 - 파라미터 최적화 버전
...
@dataclass
class RiskConfig:
    risk_per_trade: float = 0.002     # 0.2% of equity (감소)
    atr_stop_mult: float = 2.0        # 2.0x ATR (증가)
    atr_take_mult: float = 1.5        # 1.5x ATR (증가)
    fee_rate: float = 0.0004          # 0.04% per side
    slippage_rate: float = 0.0002     # 0.02% per side
    max_holding_bars: int = 12        # 12 bars (1시간, 감소)
...
class StrategyConfig:
    ...
    rsi_period: int = 14
    bb_period: int = 20
    bb_std: float = 1.8              # 1.8 (감소)
    z_period: int = 20
    ranging_adx_threshold: float = 25.0  # 25.0 (증가)
    trending_adx_threshold: float = 30.0  # 30.0 (증가)
    bandwidth_quantile_window: int = 100
    bandwidth_ranging_quantile: float = 0.4  # 0.4 (증가)
    rsi_overbought: float = 65.0     # 65.0 (감소)
    rsi_oversold: float = 35.0        # 35.0 (증가)
    z_entry_threshold: float = 1.8      # 1.8 (감소)
    volume_lookback: int = 20
    low_volume_factor: float = 0.7     # 0.7 (증가)
    tr_spike_atr_mult: float = 2.0     # 2.0 (증가)
    ...
    def _signal_row(self, row: pd.Series) -> pd.Series:
        ...
        if pd.notna(close) and pd.notna(lower) and pd.notna(rsi):
            if close <= lower and rsi <= self.cfg.rsi_oversold:
                ...
                rsi_score = min(1.0, max(0.0, (self.cfg.rsi_oversold - rsi) / 30.0))
                adx_score = 0.0 if pd.isna(adx) else min(1.0, max(0.0, (self.cfg.ranging_adx_threshold - adx) / 25.0))
                conf = self._confidence_clip(0.40 + 0.35 * rsi_score + 0.15 * distance_score + 0.10 * adx_score)
                return pd.Series(
                    {
                        "signal": "BUY",
                        "signal_source": "RANGE",
        ...
        if pd.notna(close) and pd.notna(upper) and pd.notna(rsi):
            if close >= upper and rsi >= self.cfg.rsi_overbought:
                ...
                rsi_score = min(1.0, max(0.0, (rsi - self.cfg.rsi_overbought) / 35.0))
                adx_score = 0.0 if pd.isna(adx) else min(1.0, max(0.0, (self.cfg.ranging_adx_threshold - adx) / 25.0))
                conf = self._confidence_clip(0.40 + 0.35 * rsi_score + 0.15 * distance_score + 0.10 * adx_score)
                return pd.Series(
                    {
        ...
        if pd.notna(z):
            if z <= -self.cfg.z_entry_threshold:
                z_score = min(1.0, abs(z) / (self.cfg.z_entry_threshold + 1.2))
                pb_score = 0.0 if pd.isna(pb) else min(1.0, max(0.0, (0.15 - pb) / 0.15))
                conf = self._confidence_clip(0.35 + 0.50 * z_score + 0.15 * pb_score)
                return pd.Series(
                    {
                        "signal": "BUY",
            ...
            if z >= self.cfg.z_entry_threshold:
                z_score = min(1.0, abs(z) / (self.cfg.z_entry_threshold + 1.2))
                pb_score = 0.0 if pd.isna(pb) else min(1.0, max(0.0, (pb - 0.85) / 0.15))
                conf = self._confidence_clip(0.35 + 0.50 * z_score + 0.15 * pb_score)
                return pd.Series(
                    {
                        "signal": "SELL",
...
# 메인 실행 함수
def main():
    """메인 실행 함수 - 개선된 파라미터로 백테스트"""
    print("🎯 횡보장 전략 v3 백테스트 시작 (개선된 파라미터)")
    print("=" * 60)
    ...
    # 데이터 일부만 사용 (최근 1년)
    if len(df) > 100000:  # 약 1년 분량 (5분데이터)
        df = df.tail(100000)
        print(f"📊 최근 1년 데이터만 사용: {len(df)} 행")
    ...
    output_dir = f"backtest_results_improved_{timestamp}"
