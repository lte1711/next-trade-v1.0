#!/usr/bin/env python3
"""
데이터 반영 간격 정밀 분석 (수정 버전 - FACT ONLY)
"""
...
def analyze_refresh_interval():
    ...
    if equity_cache_match:
        ...
        if equity_cache_time > base_interval_sec:
            ...
            max_cache_delay = max(max_cache_delay, equity_cache_time)
    
    if kst_cache_match:
        kst_cache_time = int(kst_cache_match.group(1))
        if kst_cache_time > base_interval_sec:
            cache_delays.append(f"KST 캐시: {kst_cache_time}초")
            max_cache_delay = max(max_cache_delay, kst_cache_time)
    
    if cache_delays:
        print(f"  - 캐시로 인한 지연:")
        for delay in cache_delays:
            print(f"    * {delay}")
        print(f"  - 실제 반영 간격: 최대 {max_cache_delay}초")
    else:
