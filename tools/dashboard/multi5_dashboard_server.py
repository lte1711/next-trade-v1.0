...
def build_kst_daily_symbol_stats(limit: int = 12) -> dict[str, Any]:
    ...
    if now - float(_KST_DAILY_SYMBOL_CACHE.get("ts", 0.0)) < 10:
        return {
            ...
            "kpi_win_rate": "-",
            "kpi_drawdown": "-",
            "engine_status": runtime.get("engine_status", "UNKNOWN"),
            "current_operation_status": "-",
            "selected_symbol": "-",
            "active_symbol_count": str(len(runtime.get("active_symbols", []))),
            "account_equity": str(snapshot.get("account_equity", "0.0")),
            "invested_margin": "-",
            "unrealized_pnl_live": "-",
            "invested_return_pct": "-",
            "exchange_open_position_count": str(len(runtime.get("active_symbols", []))),
            "ops_health_status": "-",
            "engine_alive": "true" if runtime.get("engine_alive") else "false",
            "runtime_alive": "true" if runtime.get("runtime_alive") else "false",
            "scan_symbol_count": str(len(runtime.get("active_symbols", []))),
            "target_scan_symbols": "-",
            "edge_score": "-",
            "universe_symbol_count": str(len(runtime.get("active_symbols", []))),
            "total_exposure": "-",
            "runtime_last_ts": "-",
            "active_symbols": "-",
            "universe_symbols_sample": "-",
            ...
            "exchange_api_ok": "-",
            "position_sync_ok": "-",
            "effective_role_count": str(len(runtime.get("active_symbols", []))),
            "raw_runtime_process_count": str(runtime.get("raw_runtime_process_count", 0)),
            "api_process_count": str(runtime.get("raw_runtime_process_count", 0)),
            "api_python_process_count": str(runtime.get("raw_runtime_process_count", 0)),
        }
